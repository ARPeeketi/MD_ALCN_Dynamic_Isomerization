...
filename = 'Data_TTPDLO.dat'
A=np.genfromtxt(filename,skip_header=0,skip_footer=2)

filename = 'Data_Diheds.dat'
Di=np.genfromtxt(filename,skip_header=0,skip_footer=2)
Di[:,1]=Di[:,1]/1000.0
Na = A.shape[0]
# print(Na)
B=np.zeros([Na,1])
...
filename = 'Data_TPD.dat'
fid = open(filename,'w')
line = 'Total Time %8.4f ps' % Ttime +  ' Total Iters %d\n' % N
fid.write(line)
line = 'Time        - %12.4f %12.4f\n' % (np.mean(B[0:10*wi,0]),np.mean(B[-wi:,0]))
fid.write(line)
line = 'Temperature - %12.4f %12.4f\n' % (np.mean(A[0:10*wi,2]),np.mean(A[-wi:,2]))
fid.write(line)
line = 'Pressure    - %12.4f %12.4f\n' % (np.mean(A[0:10*wi,3]),np.mean(A[-wi:,3]))
fid.write(line)
line = 'Density     - %12.4f %12.4f\n' % (np.mean(A[0:10*wi,4]),np.mean(A[-wi:,4]))
fid.write(line)
line = '%% Density change  - %12.4f' % (Dchange)
fid.write(line)
line = 'Order       - %12.4f %12.4f\n' % (np.mean(A[0:10*wi,11]),np.mean(A[-wi:,11]))
fid.write(line)
line = '%% Order change    - %12.4f' % (Ochange)
fid.write(line)
line = 'Avg Dihed   - %12.4f %12.4f' % (np.mean(Di[0:10*wi,12]),np.mean(Di[-wi:,12]))
fid.write(line)
line = 'No. of cis  - %12.4f %12.4f' % (np.mean(Di[0:10*wi,14]),np.mean(Di[-wi:,14]))
fid.write(line)
line = 'Dimension_X - %12.4f %12.4f\n' % (np.mean(A[0:10*wi,8]),np.mean(A[-wi:,8]))
fid.write(line)
line = 'Dimension_Y - %12.4f %12.4f\n' % (np.mean(A[0:10*wi,9]),np.mean(A[-wi:,9]))
fid.write(line)
line = 'Dimension_Z - %12.4f %12.4f\n' % (np.mean(A[0:10*wi,10]),np.mean(A[-wi:,10]))
fid.write(line)
fid.close()
